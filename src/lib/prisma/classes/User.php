...
class User Implements IModel {
    ...
    public function __construct($pdo, $data = null) {
        ...
        if ($data) {
            ...
            $this->product = new Product($this->pdo, $data['product'] ?? null);
            $this->Post = new Post($this->pdo, $data['Post'] ?? null);
            $this->Profile = new Profile($this->pdo, $data['Profile'] ?? null);
        }
    }
    ...
    protected function includeUserRole($items) {
        if (empty($items)) {
            return $items;
        }
        ...
        foreach ($items as &$item) {
            $item['userRole'] = $indexedRecords[$item['roleId']] ?? null;
        }

        return $singleRecord ? reset($items) : $items;
    }

    protected function includeProduct($items) {
        if (empty($items)) {
            return $items;
        }
        ...
        $dbType = $this->pdo->getAttribute(\PDO::ATTR_DRIVER_NAME);
        $quotedTableName = $dbType == 'pgsql' ? "\"Products\"" : "`Products`";
        $columnName = $dbType == 'pgsql' ? "\"id\"" : "`id`";
        ...
        foreach ($items as &$item) {
            $item['product'] = $indexedRecords[$item['productId']] ?? null;
        }

        return $singleRecord ? reset($items) : $items;
    }

    protected function includePost($items) {
        if (empty($items)) {
            return $items;
        }
        ...
        foreach ($items as &$item) {
            $item['Post'] = $indexedRecords[$item['authorId']] ?? null;
        }

        return $singleRecord ? reset($items) : $items;
    }

    protected function includeProfile($items) {
        if (empty($items)) {
            return $items;
        }
        ...
        foreach ($items as &$item) {
            $item['Profile'] = $indexedRecords[$item['userId']] ?? null;
        }
    ...
    /**
     * Finds a unique record in the 'Users' table based on provided criteria.
     ...
     * ];
     * $userRecord = $prisma->User->findUnique($uniqueCriteria);
     * if ($userRecord) {
    ...
    public function findUnique($criteria, $format = 'array')
    {
        ...
        if (isset($criteria['email'])) {
            ...
            $validatedValue = Validator::validateString($criteria['email']);
    ...
    /**
     * Retrieves multiple records from the 'Users' table based on the provided criteria.
    ...
    /**
     * Finds the first record in the 'Users' table that matches the provided criteria, with support for ordering, including relations, and selecting specific fields.
     * 
     ...
     *     'select' => ['id' => true, 'name' => true]
     * ];
     * $firstUser = $prisma->User->findFirst($criteria);
     * if ($firstUser) {
    ...
    /**
     * Updates a record in the 'Users' table identified by a unique identifier.
     * 
    ...
    /**
     * Deletes a record from the 'Users' table identified by a unique identifier.
     * 
    ...
    /**
     * Upserts a record in the 'Users' table.
     * 
    ...
    /**
     * Updates multiple records in the 'Users' table based on provided criteria.
     * 
     ...
     * $updatedRows = $prisma->User->updateMany($criteria, $updateData);
     */
    ...
    /**
     * Deletes multiple records from the 'Users' table based on provided criteria.
     * 
     ...
     * $deletedRows = $prisma->User->deleteMany($deleteCriteria);
     */
