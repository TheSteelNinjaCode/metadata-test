...
class Product Implements IModel {
    ...
    public function setPrice($value) {
        $validatedValue = Validator::validateFloat($value);
    ...
    protected function includeUser($items) {
        ...
        foreach ($items as &$item) {
            $item['User'] = $indexedRecords[$item['productId']] ?? null;
        }
    ...
    /**
     * Finds a unique record in the 'Products' table based on provided criteria.
     ...
     * ];
     * $userRecord = $prisma->Product->findUnique($uniqueCriteria);
     * if ($userRecord) {
    ...
    /**
     * Retrieves multiple records from the 'Products' table based on the provided criteria.
    ...
    public function findMany($criteria = [], $format = 'array') {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new Product($this->pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    /**
     * Finds the first record in the 'Products' table that matches the provided criteria, with support for ordering, including relations, and selecting specific fields.
     * 
     ...
     *     'select' => ['id' => true, 'name' => true]
     * ];
     * $firstUser = $prisma->Product->findFirst($criteria);
     * if ($firstUser) {
    ...
    /**
     * Updates a record in the 'Products' table identified by a unique identifier.
     * 
    ...
    /**
     * Deletes a record from the 'Products' table identified by a unique identifier.
     * 
    ...
    /**
     * Performs an aggregate operation on the 'Products' table.
     * 
    ...
    /**
     * Groups records in the 'Products' table and performs aggregate operations.
     * 
    ...
    /**
     * Updates multiple records in the 'Products' table based on provided criteria.
     * 
     ...
     * $updatedRows = $prisma->Product->updateMany($criteria, $updateData);
     */
    ...
    /**
     * Deletes multiple records from the 'Products' table based on provided criteria.
     * 
     ...
     * $deletedRows = $prisma->Product->deleteMany($deleteCriteria);
     */
    ...
    /**
     * Counts the number of records in the 'Products' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->Product->count($countCriteria);
     */
