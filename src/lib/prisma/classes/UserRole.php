...
class UserRole Implements IModel {
    ...
    protected function includeUser($items) {
        ...
        foreach ($items as &$item) {
            $item['user'] = $indexedRecords[$item['roleId']] ?? null;
        }
    ...
    /**
     * Finds a unique record in the 'UserRole' table based on provided criteria.
     ...
     * ];
     * $userRecord = $prisma->UserRole->findUnique($uniqueCriteria);
     * if ($userRecord) {
    ...
    public function findUnique($criteria, $format = 'array')
    {
        ...
        if (isset($criteria['id'])) {
            $conditions[] = "id = :id";
            $validatedValue = Validator::validateInt($criteria['id']);
            $bindings[':id'] = $validatedValue;
        }
        if (isset($criteria['name'])) {
            $conditions[] = "name = :name";
            $validatedValue = Validator::validateString($criteria['name']);
    ...
    /**
     * Retrieves multiple records from the 'UserRole' table based on the provided criteria.
    ...
    public function findMany($criteria = [], $format = 'array') {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new UserRole($this->pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    /**
     * Finds the first record in the 'UserRole' table that matches the provided criteria, with support for ordering, including relations, and selecting specific fields.
     * 
     ...
     *     'select' => ['id' => true, 'name' => true]
     * ];
     * $firstUser = $prisma->UserRole->findFirst($criteria);
     * if ($firstUser) {
    ...
    /**
     * Updates a record in the 'UserRole' table identified by a unique identifier.
     * 
    ...
    /**
     * Deletes a record from the 'UserRole' table identified by a unique identifier.
     * 
    ...
    /**
     * Upserts a record in the 'UserRole' table.
     * 
    ...
    /**
     * Performs an aggregate operation on the 'UserRole' table.
     * 
    ...
    /**
     * Updates multiple records in the 'UserRole' table based on provided criteria.
     * 
     ...
     * $updatedRows = $prisma->UserRole->updateMany($criteria, $updateData);
     */
    ...
    /**
     * Deletes multiple records from the 'UserRole' table based on provided criteria.
     * 
     ...
     * $deletedRows = $prisma->UserRole->deleteMany($deleteCriteria);
     */
