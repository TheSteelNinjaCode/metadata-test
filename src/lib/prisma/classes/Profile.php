...
class Profile Implements IModel {
    ...
    public function __construct($pdo, $data = null) {
        ...
        if ($data) {
            ...
            $this->user = new User($this->pdo, $data['user'] ?? null);
        }
    }
    ...
    protected function includeUser($items) {
        if (empty($items)) {
            return $items;
        }
        ...
        foreach ($items as &$item) {
            $item['user'] = $indexedRecords[$item['userId']] ?? null;
        }
    ...
    protected function fetchCreatedRecord($lastInsertId, $select = null, $include = null) {
        $dbType = $this->pdo->getAttribute(\PDO::ATTR_DRIVER_NAME);
        $quotedTableName = $dbType == 'pgsql' ? "\"Profiles\"" : "`Profiles`";
        $fieldsToSelect = '*';
    ...
    /**
     * Finds a unique record in the 'Profiles' table based on provided criteria.
     ...
     * ];
     * $userRecord = $prisma->Profile->findUnique($uniqueCriteria);
     * if ($userRecord) {
    ...
    public function findUnique($criteria, $format = 'array')
    {
        ...
        if (isset($criteria['userId'])) {
            ...
            $validatedValue = Validator::validateString($criteria['userId']);
            $bindings[':userId'] = $validatedValue;
        }
        if (empty($conditions)) {
    ...
    /**
     * Retrieves multiple records from the 'Profiles' table based on the provided criteria.
    ...
    public function findMany($criteria = [], $format = 'array') {
        ...
        if ($format === 'object') {
            $result = [];
            foreach ($items as $item) {
                $result[] = new Profile($this->pdo, $item); // Convert each item into a User object.
            }
            return $result;
        }
    ...
    /**
     * Finds the first record in the 'Profiles' table that matches the provided criteria, with support for ordering, including relations, and selecting specific fields.
     * 
     ...
     *     'select' => ['id' => true, 'name' => true]
     * ];
     * $firstUser = $prisma->Profile->findFirst($criteria);
     * if ($firstUser) {
    ...
    /**
     * Updates a record in the 'Profiles' table identified by a unique identifier.
     * 
    ...
    /**
     * Deletes a record from the 'Profiles' table identified by a unique identifier.
     * 
    ...
    /**
     * Upserts a record in the 'Profiles' table.
     * 
    ...
    /**
     * Performs an aggregate operation on the 'Profiles' table.
     * 
    ...
    /**
     * Groups records in the 'Profiles' table and performs aggregate operations.
     * 
    ...
    /**
     * Updates multiple records in the 'Profiles' table based on provided criteria.
     * 
     ...
     * $updatedRows = $prisma->Profile->updateMany($criteria, $updateData);
     */
    ...
    /**
     * Deletes multiple records from the 'Profiles' table based on provided criteria.
     * 
     ...
     * $deletedRows = $prisma->Profile->deleteMany($deleteCriteria);
     */
    ...
    /**
     * Counts the number of records in the 'Profiles' table based on provided criteria.
     * 
     ...
     * $activeUserCount = $prisma->Profile->count($countCriteria);
     */
